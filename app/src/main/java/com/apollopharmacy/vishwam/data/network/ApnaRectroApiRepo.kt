...
object ApnaRectroApiRepo {
    ...
    suspend fun getStorePendingAndApprovedListApnaRetro(
        ...
        return try {
            val response = Api.getClient().getStorePendingAndApprovedListApnaRetro(url,token,getpendingAndApprovedListRequest)
    ...
    suspend fun getNotificationList(
        url: String,
    ): ApiResult<NotificationModelResponse> {
        return try {
            val response = Api.getClient().getNotificationsList(url)
            ApiResult.Success(response)
        } catch (e: Exception) {
            ApiResult.UnknownError(e.message)
        } catch (e: IOException) {
