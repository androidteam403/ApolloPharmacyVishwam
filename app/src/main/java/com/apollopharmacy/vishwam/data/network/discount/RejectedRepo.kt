...
object RejectedRepo {
    ...
    suspend fun getRejectedList(
        pendingOrderRequest: PendingOrderRequest,
        url: String,
    ): ApiResult<RejectedOrderResponse> {
        return try {
            val encriptData =
                EncryptionManager.encryptData(Gson().toJson(pendingOrderRequest), ENCRIPTION_KEY)
            val response =
                Api.getClient().getRejectedList(url, CommonRequest(encriptData.toString()))
            val decriptData = EncryptionManager.decryptData(response, ENCRIPTION_KEY)
            val Responsee = Gson().fromJson(decriptData, RejectedOrderResponse::class.java)
            if (Responsee.STATUS)
