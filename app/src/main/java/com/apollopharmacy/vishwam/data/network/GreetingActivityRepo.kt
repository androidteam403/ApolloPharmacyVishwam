...
object GreetingActivityRepo {
    suspend fun employeeWishesApiCAll(
        url: String,
        employeeWishesRequest: EmployeeWishesRequest?,
    ): ApiResult<EmployeeWishesResponse> {
        return try {
            val response = Api.getClient().EMPLOYEE_WISHES_API_CALL(url, employeeWishesRequest)
            if (response.insertEMPResult != null) ApiResult.Success(response)
