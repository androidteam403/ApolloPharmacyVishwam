...
public class PrintfTSPLManager {
    ...
    public static PrintfTSPLManager getInstance(final Context context) {
        if (PrintfLabelManagerHolder.instance.context == null) {
            PrintfLabelManagerHolder.instance.context
   ...
   /* public void printQrCode(int x, int y, String level, int width, int rotation, String content) {
    ...
    public void printQrCode(int x, int y, String level, int width, int rotation, String content) {
        byte[] bytes = new byte[0];
        try {
            String contentWithImage = "QRCODE " + x + "," + y + "," + level + "," + width + ",M," + rotation + ",M1,S1,\"" + content + "\"\n";

// Convert the content to bytes using GBK encoding
            bytes = contentWithImage.getBytes("GBK");
