...
class CeoDashboardViewModel : ViewModel() {
    ...
    fun getTicketListByCountApi(
        ...
        //"https://cmsuat.apollopharmacy.org/zc-v3.1-user-svc/2.0/apollo_cms/api/ticket/list/get-ticket-counts-by-status-role?"
        var getTicketListByCounttoken = ""
        for (i in data.APIS.indices) {
            if (data.APIS[i].NAME.equals("MONITORING TICKET COUNTS BY STATUS ROLE")) {
                getTicketListByCountUrl = data.APIS[i].URL
                getTicketListByCounttoken = data.APIS[i].TOKEN
                break
            }
        }
        ...
        viewModelScope.launch {
            val result = withContext(Dispatchers.IO) {
                RegistrationRepo.getDetails(
                    baseUrl, token, GetDetailsRequest(getTicketListByCountUrl, "GET", "The", "", "")
                )
            }
            when (result) {
