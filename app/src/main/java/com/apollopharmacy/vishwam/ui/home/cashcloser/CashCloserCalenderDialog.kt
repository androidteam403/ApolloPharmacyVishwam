...
class CashCloserCalenderDialog : DialogFragment() {
...
//            val _month = if (_month + 1 < 10) 0 + (_month + 1) else (_month + 1)
        ...
        if (arguments?.getBoolean(KEY_IS_TO) == true) {
            val date = SimpleDateFormat("dd-MMM-yyyy", Locale.ENGLISH).parse(
                arguments?.getString(KEY_FROM_DATE)
            )
...
//            if (arguments?.getBoolean(KEY_IS_TO) == true) {
            ...
            /*} else {
                dateSelectedListner!!.selectedDatefrom(getDate, getDate)
