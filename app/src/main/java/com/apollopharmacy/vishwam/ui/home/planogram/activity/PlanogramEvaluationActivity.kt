...
class PlanogramEvaluationActivity : AppCompatActivity(), PlanogramActivityCallback,
    ...
    override fun onClickSubmit() {
        ...
        if(areasToFocusOnList!=null && areasToFocusOnList!!.size>0){
            for (i in areasToFocusOnList!!.indices) {
                ...
                if (!commaSeparatorAreasToFocusOn.isEmpty()) {
                    commaSeparatorAreasToFocusOn = indiviualCat + "," + commaSeparatorAreasToFocusOn
                } else {
        ...
        planogramRequest.branchName = activityPlanogramEvaluationBinding.storeName.text.toString()
        ...
        planogramRequest.overallScore= planogramSurveyQuestionsListResponses!!.overAllScore.toString()
            ...
            if (i.type.equals("category")){
                ...
                for( j in i.questions!!){
                    ...
                    if(j.name.equals("Customer facing")){
                        ...
                        var groupedProductWise = planogramPeghooksDisplay.GroupedProductWise__2()
                        var uid = j.value
                        groupedProductWise.uid=uid
...
//        dialogSubmit.show()
    ...
    fun caluclateOverAllScore() {
        ...
        for (i in planogramSurveyQuestionsListResponses!!.data!!.listData!!.rows!!) {
            ...
            if (havingAllCategoryScore) {
                ...
                var overAllScore = (valueOfTotalCategories / totalCategories) * 100
                planogramSurveyQuestionsListResponses!!.overAllScore = overAllScore.toFloat()
            }
        }
    }
    ...
    override fun onSuccessSaveUpdateApi(saveUpdateRequestJsonResponse: PlanogramSaveUpdateResponse?) {
        Toast.makeText(applicationContext, ""+ saveUpdateRequestJsonResponse!!.message, Toast.LENGTH_SHORT).show()
        Utlis.hideLoading()
    }
