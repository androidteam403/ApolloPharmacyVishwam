...
import com.apollopharmacy.vishwam.databinding.DialogStateBinding
...
class StateDialog : DialogFragment() {
    val TAG = "StateDialog"
    ...
    lateinit var viewBinding: DialogStateBinding
    lateinit var stateRecyclerView: StateRecyclerView

    interface AbstractDialogItemClickListner {
        fun selectState(item: String)
    }
    ...
    override fun onCreateView(
        ...
        viewBinding = DialogStateBinding.inflate(inflater, container, false)
...
//        })
        ...
        items.add("Andhra Pradesh")
        items.add("Karnatak")
        items.add("Maharastra")
        items.add("Odisha")

        stateRecyclerView = StateRecyclerView(items, object : OnSelectedListnerState {
            override fun onSelected(data: String) {
                abstractDialogClick.selectState(data)
                dismiss()
            }
        })
        viewBinding.itemTypeRecyclerView.adapter = stateRecyclerView
...
class StateRecyclerView(
    ...
    var onSelectedListner: OnSelectedListnerState
) :
...
interface OnSelectedListnerState {
